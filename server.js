...
(function() {
  var DEFAULT_HZ, Canvas, GIFEncoder, Image, app, currentImg, drone, Rx, encoder, express, faye, flushRecentImgs, fs, imageSendingPaused, maxRecentImgs, path, recentImgs, server, socket;
  ...
  var demoDataStream = Rx.Observable.fromEvent(drone, 'navdata')
    .filter(function (navData) {
      return navData.demo;
    })
    .pluck('demo');

  Rx.Observable.prototype.smooth = function (field) {
    return this
      .pluck(field)
      .map(function (data) {
          console.log(data);
